...
## Scene-referred rendering path

Since 2026-09-10 this is the main line (the previous pipeline is kept on the `stock-pipeline`
branch and the `stock-pipeline-base` tag). The scene-referred rendering path: with the camera's **Scene Referred**
...
an ambient light asked to cast shadows). Besides the plugin and the renderer the path touches
