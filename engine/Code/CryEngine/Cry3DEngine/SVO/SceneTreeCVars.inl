...
REGISTER_CVAR_AUTO(float, e_svoTI_RT_SafetyBorder, 0.25f, VF_EXPERIMENTAL, "DEPRECATED - retired together with the per-voxel triangle list path\nKept registered only so that existing level files keep loading");
...
REGISTER_CVAR_AUTO(int, e_svoTI_RT_UploadBudgetMB, 8, VF_EXPERIMENTAL, "Megabytes of ray tracing pool data uploaded to the GPU per rendered frame\nThe upload runs once per frame and copies every submitted byte into a render thread command, so this is also the per frame main thread allocation it costs; dirty slices that do not fit are carried to the next frame, oldest first\n0 = one slice per frame (the smallest step that still makes progress)");
REGISTER_CVAR_AUTO(int, e_svoTI_RT_DynRefreshFrames, 1, VF_EXPERIMENTAL, "Frames the dynamic object query result is reused for before the octree is walked again (the camera moving more than a twentieth of e_svoTI_RT_MaxDistCam also forces a re-query)\n1 = query every frame\nWARNING: above 1 the collected IRenderNode pointers are held across frames and an object destroyed in between leaves a dangling pointer - raise it only in a scene where nothing is deleted");
REGISTER_CVAR_AUTO(int, e_svoTI_RT_TexPrefetch, 1, VF_EXPERIMENTAL, "Load and decompress the low resolution material texture copies on one dedicated worker instead of synchronously on every voxelization job\nA material whose copies are not ready yet is emitted with no texture and is patched in place once they arrive\n0 = the old synchronous behaviour");
REGISTER_CVAR_AUTO(int, e_svoTI_RT_MaxBounces, 1, VF_EXPERIMENTAL, "Maximum number of ray traced bounces");
