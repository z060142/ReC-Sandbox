...
int CRendererCVars::CV_r_SceneReferredExportCurves;
...
//! r_SceneReferredExrSelfTest: writes a 4x4 EXR and reads it back with tinyexr's own loader
//! (Stage S6 item 1). It runs from the change callback so that setting the cvar - in the console,
//! or from a config applied at start-up - IS the trigger; there is no unit-test framework here and
//! this needs none. Setting it back to 0 does nothing.
...
void CRendererCVars::InitCVars()
{
	...
	REGISTER_CVAR3("r_SceneReferred", CV_r_SceneReferred, 1, VF_NULL,
	               "Kill switch for the SCENE-REFERRED pipeline (SceneReferredSpec.md), which keeps "
	               "everything between the lights and the display transform in scene units - real "
	               "luminance, no [0,1], no display encoding - so exposure, white balance, grading "
	               "and every optical effect operate on how much light is in the scene rather than "
	               "on a picture that has already been squeezed onto a monitor. It runs ONLY when "
	               "the Cinematic Camera component asks for it (Scene Referred). "
	...
	REGISTER_CVAR3("r_SceneReferredStage", CV_r_SceneReferredStage, 99, VF_NULL,
	               "BISECTION switch for the scene-referred pipeline. r_SceneReferred is the master "
	               "kill switch; this one honours the camera's request only up to a given stage, so "
	               "that a bad frame can be walked back to the single item that causes it without "
	               "rebuilding anything. Set it BEFORE ticking Scene Referred on the component, then "
	               "raise it one step at a time. "
	               "0 = the switch and nothing else: the request is published and logged, the whole "
	               "renderer stays stock. "
	               "1 = + S0, the fp16 floor on the HDR targets (render-target formats move). "
	               "2 = + S1, the camera's pre-exposure and the AUTO metering. "
	               "3 = + S2, the radiometric light-unit convention (1/PI in the BRDF, sun not "
	               "pre-divided, sky scale) and the fp16 sun-shaft working set. "
	               "4 = + S3, the Rec.709 -> ACEScg working-space conversion. "
	               "5 = + S4/S5/S7, the output transform and the camera grade: the ODT and look "
	               "LUTs, the inert stock LDR stage, and thresholds read as stops above mid grey. "
	               "6 = + S6, the EXR export tap. "
	               "99 = everything (default). Values above 6 all mean the same thing.");
	...
	REGISTER_CVAR3("r_SceneReferredMeterHistogram", CV_r_SceneReferredMeterHistogram, 1, VF_NULL,
	               "HOW the Cinematic Camera's AUTO mode meters, while the scene-referred path is "
	               "active (stage 2 and up). "
	               "1 = the 64-bin log2 LUMINANCE histogram (default): a compute pass over the "
	               "quarter-res exposed frame bins every texel by how many stops it sits above or "
	               "below mid grey, weighted by the camera's Metering Mode, and the camera takes "
	               "the log-mean of a percentile band of it. Robust by construction to one large "
	               "region owning the average, which a mean is not - an outdoor sky is half the "
	               "frame and it is not the subject. "
	               "0 = the shipped whole-frame log-mean of the albedo-divided illuminance "
	...
	REGISTER_CVAR3("r_SceneReferredExportCurves", CV_r_SceneReferredExportCurves, 1, VF_NULL,
	               "Write the CineCam Grade's two 1D curves beside a capture. 1 = yes (default), 0 = no. "
	               "The ASC CDL cannot carry them, so they travel as their own files, once per TAKE and "
	               "ONLY when a curve is doing something (unlike the .cdl, an absent file here is not "
	               "ambiguous - the .cdl beside it always dates the take). "
	               "<prefix>.curve_master.cube is the master (luma) curve as a Resolve 1D LUT, which "
	               "applies mechanically on a node AFTER the .cdl node. <prefix>.curves.txt carries both "
	               "curves' control points in plain text, because Sat vs Sat is a multiplier indexed by a "
	               "per-pixel saturation measure and NOT a per-channel value mapping - a .cube of it would "
	               "load, apply and be silently nonsense. Resolve has that control natively; the file says "
	               "exactly how the measure is defined so the numbers can be retyped into it.");
	...
	REGISTER_CVAR3_CB("r_SceneReferredExrSelfTest", CV_r_SceneReferredExrSelfTest, 0, VF_NULL,
	               ...
	               "tinyexr's own loader, and logs whether the pixels round-tripped EXACTLY (half "
	               "in, half out - there is no conversion anywhere on this path, so anything but "
	               "an exact match is a bug) and whether all three header attributes survived the "
	               "write. It is the one check that says the library is linked, its ZIP codec is "
	               "talking to zlib rather than to a missing miniz, and its WRITER really emits "
	               "custom attributes - on which every piece of camera metadata in an exported "
	               "frame depends. 0 = off (default); the test runs on the transition to non-zero.",
	               OnChange_CV_r_SceneReferredExrSelfTest);
