...
void CSkyStage::Execute(CTexture* pColorTex, CTexture* pDepthTex)
{
	...
	if (isProcedualSky)
	{
		...
		if (starIntensity > 1e-3f && m_isStarsDataLoaded)
		{
			...
			const uint64 depthState = bReverseDepth ? GS_DEPTHFUNC_GEQUAL : GS_DEPTHFUNC_LEQUAL;
