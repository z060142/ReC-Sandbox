...
if(NOT OPTION_ENGINE OR OPTION_DEDICATED_SERVER)
	set(OPTION_SCALEFORM3 OFF)
	set(OPTION_SCALEFORM4 OFF)
else()
	# No Scaleform SDK (Choose whether the engine uses Scaleform at all)
	if (NOT EXISTS "${SDK_DIR}/Scaleform" AND NOT EXISTS "${SDK_DIR}/Scaleform4")
		set(OPTION_SCALEFORM ON CACHE BOOL "Support Scaleform via helper modules")

		unset(OPTION_SCALEFORM3 CACHE)
		unset(OPTION_SCALEFORM4 CACHE)
		set(OPTION_SCALEFORM3 OPTION_SCALEFORM)
		set(OPTION_SCALEFORM4 OPTION_SCALEFORM)
	else()
		# Monolithic Builds (Can only have a single Scaleform implementation built-in until monolithic is fixed)
		if (OPTION_STATIC_LINKING)
			if (NOT DEFINED OPTION_SCALEFORM)
				set(OPTION_SCALEFORM "SCALEFORM4" CACHE STRING "Scaleform version to use in a monolithic build")
				set(OPTION_SCALEFORM_OPTIONS "SCALEFORM3" "SCALEFORM4")
				set_property(CACHE OPTION_SCALEFORM PROPERTY STRINGS ${OPTION_SCALEFORM_OPTIONS})
			endif()

			unset(OPTION_SCALEFORM3 CACHE)
			unset(OPTION_SCALEFORM4 CACHE)

			set("OPTION_$CACHE{OPTION_SCALEFORM}" ON CACHE BOOL "" INTERNAL)
		else()
			# Normal module builds
			if(DEFINED OPTION_SCALEFORM)
				set_property(CACHE OPTION_SCALEFORM PROPERTY STRINGS)
				unset(OPTION_SCALEFORM CACHE)
			endif()
...
if(ORBIS)
	...
	option(GNM_VALIDATION "Enable GNM validation" OFF)
endif()
...
if (OPTION_STATIC_LINKING)
	if (NOT DEFINED AUDIO_IMPL)
		set(AUDIO_IMPL "FMOD" CACHE STRING "Audio library to use in a monolithic build")
		set(AUDIO_IMPL_OPTIONS "ADX2" "FMOD" "PORTAUDIO" "SDL_MIXER" "WWISE")
		set_property(CACHE AUDIO_IMPL PROPERTY STRINGS ${AUDIO_IMPL_OPTIONS})
	endif()

	unset(AUDIO_ADX2 CACHE)
	unset(AUDIO_FMOD CACHE)
	unset(AUDIO_PORTAUDIO CACHE)
	unset(AUDIO_SDL_MIXER CACHE)
	unset(AUDIO_WWISE CACHE)

	set("AUDIO_$CACHE{AUDIO_IMPL}" ON CACHE BOOL "" INTERNAL)
elseif(DEFINED AUDIO_IMPL)
	set_property(CACHE AUDIO_IMPL PROPERTY STRINGS)
	unset(AUDIO_IMPL CACHE)
endif()
...
elseif(NOT OPTION_STATIC_LINKING)
...
if (OPTION_SCALEFORM3 AND NOT (OPTION_ENGINE OR OPTION_SHADERCACHEGEN OR OPTION_DOXYGEN_EXAMPLES))
...
if (OPTION_ENGINE OR OPTION_SHADERCACHEGEN)
	...
	# S6 (SceneReferredSpec.md): the EXR writer behind rec_CaptureEXR. Same guarded block as
	# tiff/lz4, so no unguarded configuration picks it up.
	add_subdirectory ("Code/Libs/tinyexr")
endif()
...
add_subdirectory ("Code/Libs/jpeg")
